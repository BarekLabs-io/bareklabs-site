...
import { TAPE_SYMBOLS, MOVERS_SYMBOLS, DESK_SYMBOLS } from '@/data/marketTape'
...
export function MarketQuotesProvider({ children }: { children: ReactNode }) {
  ...
  const value = useLiveQuotes(Array.from(new Set([...TAPE_SYMBOLS, ...MOVERS_SYMBOLS, ...DESK_SYMBOLS])))
