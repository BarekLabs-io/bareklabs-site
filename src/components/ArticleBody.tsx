...
export default function ArticleBody({ content }: { content: string }) {
  return (
    <ReactMarkdown
      ...
      components={{
        h2: ({ children }) => (
          <h2 className="mt-16 border-t border-line pt-10 text-2xl font-medium tracking-tight first:mt-0 first:border-t-0 first:pt-0 md:text-3xl">
            {children}
          </h2>
        ),
        h3: ({ children }) => (
          <h3 className="mt-10 text-lg font-medium tracking-tight text-foreground md:text-xl">{children}</h3>
        ),
        p: ({ children }) => <p className="prose-lab mt-6 first:mt-0">{children}</p>,
        ...
        blockquote: ({ children }) => (
          <blockquote className="my-8 max-w-3xl border-s-2 border-signal bg-secondary p-6 font-mono-lab text-[13px] leading-6 tracking-wide text-dim [&_p]:mt-0 [&_p]:font-mono-lab [&_p]:text-[13px] [&_p]:leading-6 [&_p]:tracking-wide [&_p+p]:mt-3 [&_strong]:text-signal">
            {children}
          </blockquote>
        ),
        ul: ({ children }) => <ul className="prose-lab mt-6 list-disc space-y-2 ps-5 marker:text-signal">{children}</ul>,
        ol: ({ children }) => <ol className="prose-lab mt-6 list-decimal space-y-2 ps-5 marker:text-signal">{children}</ol>,
