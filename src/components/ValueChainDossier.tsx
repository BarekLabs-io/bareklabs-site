...
import { NO_VALUE, formatLevel, formatChange } from '@/data/marketTape'
...
function Quote({ ticker, quote }: { ticker: string; quote?: { price: number; changePercent: number | null } }) {
  const hasDive = !!companies[ticker]
  const up = quote?.changePercent != null ? quote.changePercent >= 0 : null
  const body = (
    <>
      <span className={cn('font-mono-lab text-[10px] tracking-wider', hasDive ? 'text-signal' : 'text-foreground/80')}>
        {ticker}
      </span>
      <span className="font-mono-lab text-[10px] tabular-nums text-dim" dir="ltr">
        {quote ? formatLevel(quote.price) : NO_VALUE}
      </span>
      {quote?.changePercent != null && (
        <span className={cn('font-mono-lab text-[9px] tabular-nums', up ? 'text-signal' : 'text-danger')} dir="ltr">
  ...
  return (
    <div className="border border-line bg-panel">
      <div className="border-b border-line px-6 py-4">
        ...
        <p className="mt-2 max-w-3xl font-mono-lab text-[10.5px] leading-5 tracking-wide text-dim">{d.intro}</p>
      </div>
      ...
      <div className="flex flex-wrap gap-px border-b border-line bg-line">
        {TABS.map((tb) => (
          <button
            ...
            className={cn(
              'flex-1 whitespace-nowrap bg-panel px-4 py-3 font-mono-lab text-[9.5px] tracking-[0.2em] transition-colors duration-300',
      ...
      <div className="p-6">
            ...
            {LAYERS.map((l) => (
              <div key={l.n} className="border border-line bg-card2 p-5" style={{ borderInlineStartWidth: 3, borderInlineStartColor: l.accent }}>
                <div className="flex flex-wrap items-baseline gap-3">
                  <span className="font-mono-lab text-[10px] tracking-[0.2em] text-signal">{l.n}</span>
                  <span className="text-base font-medium tracking-tight">{l.title}</span>
                </div>
                {l.note && <p className="mt-2 max-w-3xl font-mono-lab text-[10px] leading-5 tracking-wide text-dim">{l.note}</p>}
                <div className="mt-4 flex flex-col gap-4">
                  {l.segments.map((sg) => (
                    <div key={sg.name} className="border-t border-line pt-3">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <span className="font-mono-lab text-[11px] tracking-wide text-foreground">{sg.name}</span>
                        <MaturityTag m={sg.maturity} />
                      </div>
                      <p className="mt-1.5 max-w-2xl font-mono-lab text-[10px] leading-5 tracking-wide text-dim">{sg.role}</p>
                      <div className="mt-2.5 flex flex-wrap gap-1.5">
                        {sg.tickers.map((tk) => (
                          <Quote key={tk} ticker={tk} quote={quotes[tk]} />
                        ))}
                        {sg.privates?.map((p) => (
                          <span key={p} className="inline-flex items-center border border-dashed border-line px-2 py-1 font-mono-lab text-[10px] text-faint">
                            {p} · {d.private}
                          </span>
                        ))}
                      </div>
        ...
        {tab === 'wall' && (
          <div>
            ...
            <div className="mt-6">
              <div className="font-mono-lab text-[9px] tracking-[0.25em] text-dim">{d.timeline}</div>
              <div className="mt-3 flex flex-col gap-px overflow-hidden border border-line bg-line md:flex-row">
                {BOTTLENECKS.map((b, i) => (
                  <div key={b.period} className="flex-1 bg-card2 p-4">
                    <div className="font-mono-lab text-[9px] tracking-[0.2em] text-signal" dir="ltr">{b.period}</div>
                    <div className="mt-1.5 font-mono-lab text-[11px] tracking-wide text-foreground">{b.label}</div>
                    <p className="mt-1.5 font-mono-lab text-[9.5px] leading-4 tracking-wide text-dim">{b.detail}</p>
              ...
              {GRID_PLAYERS.map((p) => (
                <div key={p.ticker} className="border border-line bg-card2 p-4">
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <span className="font-mono-lab text-[10px] tracking-wide text-dim">{p.segment}</span>
                  </div>
        ...
        {tab === 'epc' && (
          <div>
            <p className="max-w-3xl font-mono-lab text-[10.5px] leading-5 tracking-wide text-dim">{d.epcIntro}</p>
            ...
            <div className="mt-5 overflow-x-auto">
              <table className="w-full min-w-[720px] border border-line">
                <thead>
                  <tr className="border-b border-line bg-card2 font-mono-lab text-[8.5px] tracking-[0.2em] text-faint">
            ...
            <div className="flex flex-col gap-3">
              {UNSEEN.map((u) => (
                <div key={u.name} className="border border-line bg-card2 p-4">
                  ...
                  <div className="mt-2 font-mono-lab text-[9.5px] tracking-[0.15em] text-signal">{u.craft}</div>
                  <p className="mt-1.5 font-mono-lab text-[10px] leading-5 tracking-wide text-foreground/80">{u.why}</p>
                </div>
              ))}
            </div>
            <div className="mt-6">
              <div className="font-mono-lab text-[9px] tracking-[0.25em] text-dim">{d.geo}</div>
              <div className="mt-3 grid gap-px overflow-hidden border border-line bg-line md:grid-cols-3">
                {GEO_HUBS.map((g) => (
                  <div key={g.region} className="bg-card2 p-4">
                    <div className="font-mono-lab text-[10px] tracking-wide text-signal">{g.region}</div>
                    <div className="mt-1 font-mono-lab text-[9px] tracking-[0.2em] text-faint">{g.theme}</div>
                    <p className="mt-2 font-mono-lab text-[9.5px] leading-4 tracking-wide text-dim">{g.detail}</p>
                  </div>
                ))}
              </div>
            </div>
