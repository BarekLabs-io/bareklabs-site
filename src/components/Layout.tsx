...
function Clock() {
  ...
  return (
    <div className="hidden items-center gap-3 font-mono-lab text-[10px] tracking-wider text-dim min-[1400px]:flex" dir="ltr">
    ...
    <div className="flex items-center border border-line" role="group" aria-label="Language">
      {LANG_META.map((l: { id: Lang; label: string; short: string }) => (
        <button
          ...
          title={l.label}
          className={cn(
            'px-1.5 py-1.5 font-mono-lab text-[9.5px] tracking-[0.15em] transition-all duration-300 sm:px-2 md:px-2.5',
...
function SearchButton() {
  ...
  return (
    <button
      ...
      className="hidden items-center gap-2.5 border border-line px-3 py-1.5 font-mono-lab text-[9.5px] tracking-[0.15em] text-dim transition-colors duration-300 hover:border-signal hover:text-signal lg:flex"
    >
      <span>⌕</span>
      <span className="hidden min-[1680px]:inline">{t.header.search}</span>
      <span className="hidden whitespace-nowrap border border-line px-1 py-px text-[8px] text-faint min-[1680px]:inline" dir="ltr">{t.header.searchHint}</span>
    </button>
  )
}
  ...
  if (!n.children) {
    return (
      <NavLink
        ...
        className={cn(
          'nav-link flex h-full items-center whitespace-nowrap font-mono-lab text-[12px] min-[1680px]:text-[13px] tracking-[0.12em] transition-colors',
      ...
      <header className="fixed inset-x-0 top-0 z-50">
        <div className={cn('relative z-10 border-b transition-all duration-500', scrolled ? 'border-line header-glass' : 'border-line/60 header-glass')}>
          <div className="shell flex h-24 items-stretch justify-between px-4 md:h-[92px] md:px-10">
            <Link to="/" className="flex shrink-0 items-center gap-3 me-3 md:me-6 min-[1680px]:me-10">
              <img src="/logo.svg" alt="BAREK / LABS" className="h-7 w-auto min-[375px]:h-8 md:h-11 min-[1680px]:h-[52px] logo-adaptive" />
            </Link>

            <nav className="hidden items-stretch gap-5 xl:gap-7 min-[1680px]:gap-9 lg:flex">
              {NAV.map((n) => (
                <DesktopNavItem key={n.to} n={n} />
              ))}
            </nav>

            <div className="flex items-center gap-2 md:gap-3 min-[1680px]:gap-4 md:ms-6 min-[1680px]:ms-10">
              <Clock />
              <div className="hidden items-center gap-2 whitespace-nowrap font-mono-lab text-[10px] tracking-wider text-signal min-[1680px]:flex">
          ...
          {/* mobile menu */}
          {menuOpen && (
            <nav className="border-t border-line bg-ink px-5 py-6 lg:hidden">
              {NAV.map((n) => (
                <div key={n.to}>
                  <NavLink
                    to={n.to}
